    ...
    <n-card size="huge" class="h-full shadow-sm rounded-16px">
      <n-tabs type="line" animated>
        <n-tab-pane name="oasis" tab="展示">
          ...
          >
            <n-grid-item
              v-for="(item, index) in obj.pictureArr"
              ref="picture-grid-item"
              :key="index"
              class="other-file-grid-item"
            >
              <div class="show-file-box">
                <div class="show-file-name">
                  <n-tag checkable @click="copyContent(item.file.fileName)">
                    <n-ellipsis :line-clamp="1">{{ item.file.fileName }}</n-ellipsis>
                  </n-tag>
                </div>
                <div class="show-file-format">
                  <div
                    v-if="!isImageFile(item)"
                    :class="isImageFile(item) ? 'no-show-bg' : 'show-file-bg'"
                  ...
                  <n-image v-else :show-toolbar-tooltip="true" :src="item.file.path" />
                </div>
              </div>
...
const obj = reactive({
  /** 图片文件的后缀名 */
  fileFormatOptions: new Set<string>(),
 ...
 * 获取该用户的所有文件格式
 */
const loadAllFileFormat = () => {
  obj.fileFormatOptions = new Set<string>();
  fetchAllFileFormat(auth.getAuthUserInfo.userUid).then(data => {
    data.data.forEach(value => obj.fileFormatOptions.add(value));
  });
};
    ...
    },
    {
      ...
      render(row) {
        return h(
          ...
          {
            checkable: true,
            onClick: () => copyContent(row.file.uid as string)
          },
          {
            default: () => row.file.uid
          }
        );
      }
    },
...
const isImageFile = computed(() => {
  return (fileInfo: PictureInfo) => {
    return ['png', 'jpg', 'jpeg'].includes(fileInfo.file?.fileName?.split('.')[1]);
  };
});
</script>

<style lang="less">
.other-file-grid-item {
  height: 12rem;
